...
router.post("/event/in/gruppe/erstellen", verifyToken, async (req, res)=>{
    const {eventname, ort, zeit, bemerkung, gruppenid, personid, fahrer} = req.body;
    const eventDao = new EventDao(req.app.locals.dbConnection);
    try {
        const eventid = await eventDao.eventAnlegen(eventname, ort, zeit, bemerkung, gruppenid, fahrer);
        console.log(eventid);
